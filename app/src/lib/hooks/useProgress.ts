...
function loadProgress(): UserProgress {
  ...
  try {
    console.log('📂 Carregando progresso do localStorage...');
    const stored = localStorage.getItem(STORAGE_KEY);
    if (!stored) {
      console.log('⚠️ Nenhum progresso salvo encontrado, usando dados iniciais');
...
function saveProgress(progress: UserProgress): void {
  ...
  try {
    console.log('💾 Salvando progresso no localStorage:', {
...
export function useProgress() {
  ...
  const [progress, setProgress] = useState<UserProgress>(() => {
    console.log('🔄 useProgress montado, carregando do storage...');
    return loadProgress();
  });
  ...
  useEffect(() => {
    ...
    console.log('🔄 Progresso mudou, salvando...');
  ...
  const addWords = useCallback(
    (count: number, category?: string) => {
      console.log('📝 addWords chamado com:', { count, category });

      setProgress((prev) => {
        console.log('📊 Estado anterior:', prev.stats.totalWords, 'palavras');

        const wordXP = Math.floor(count / 100) * XP_RULES.WORDS_PER_100;
        console.log('💰 XP calculado:', wordXP, 'para', count, 'palavras');
        ...
        console.log('✅ Novo estado:', newProgress.stats.totalWords, 'palavras,', newProgress.level.totalXP, 'XP');
        return newProgress;
      });
